...
// Clean the Supabase URL to remove any trailing '/rest/v1/' or '/rest/v1'
const cleanSupabaseUrl = supabaseUrl.replace(/\/rest\/v1\/?$/, "");

export const isSupabaseConfigured = !!(cleanSupabaseUrl && !cleanSupabaseUrl.includes("placeholder") && supabaseAnonKey && !supabaseAnonKey.includes("placeholder"));

// Create the Supabase client
export const supabase = createClient(
  cleanSupabaseUrl || "https://placeholder-project.supabase.co",
  supabaseAnonKey || "placeholder-anon-key"
);

/**
 * Verifies the database connection by executing a quick query.
 */
  ...
  try {
    ...
    if (error) {
      if (error.code && error.code.startsWith("PGRST")) {
        console.log("Supabase connected and authenticated successfully:", error.message);
        return true;
      }
