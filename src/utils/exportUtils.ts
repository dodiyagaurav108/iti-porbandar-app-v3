...
export function getGujaratiTradeName(englishName: string): string {
  ...
  if (normalized.includes("sewing")) return "સીવણ ટેકનોલોજી";
...
// --- ON ROLL SUMMARY ---
export function exportOnRollSummaryExcel(students: Student[], filename: string = "OnRoll_Summary") {
  const onRoll = students.filter(s => s.currentStatus === StudentStatus.ACTIVE);
  const data = onRoll.map(s => ({
    "Enrollment No": s.enrollmentNumber,
    "Trainee Name": `${s.studentName} ${s.surname}`.trim(),
    "Gender": s.gender,
    "Trade": s.trade,
    "Batch": s.batchName,
    "Admission Date": s.admissionDate || "-",
  ...
  exportToExcel(data, filename, "OnRoll_Trainees");
}

export async function exportOnRollSummaryPDF(students: Student[], filename: string = "OnRoll_Summary") {
  const onRoll = students.filter(s => s.currentStatus === StudentStatus.ACTIVE);
  const headers = [["Enrollment No", "Trainee Name", "Gender", "Trade", "Batch", "Admission Date", "Year", "Shift"]];
  const rows = onRoll.map(s => [
    s.enrollmentNumber,
    `${s.studentName} ${s.surname}`.trim(),
    s.gender,
    s.trade,
  ...
  await exportToPDF("CURRENT ON-ROLL TRAINEES DIRECTORY", headers, rows, filename);
}
